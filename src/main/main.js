...
`;
;
const BRIDGE_VERSION = 'e003commentfix';
...
function ensureBridge() {
  ...
  try {
    fs.writeFileSync(BRIDGE_FILE, BRIDGE_SOURCE + BRIDGE_ANALYZE, 'utf-8');
...
const BRIDGE_TIMEOUTS = {
  ...
  analyze: 180000,                    // batch ingest — large files can take time
  deep_analyze: 300000,               // deep content analysis — reads every file body
...
ipcMain.handle('sem:analyze', async (_, args) => {
  // Support _bridge_cmd override to route to different bridge commands
  if (args && args._bridge_cmd) {
    const { _bridge_cmd, ...cleanArgs } = args;
    return runBridge(_bridge_cmd, cleanArgs);
  }
  return runBridge('analyze', args || {});
});
ipcMain.handle('sem:deep_analyze', async (_, args) => runBridge('deep_analyze', args || {}));
